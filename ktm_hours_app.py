...
names = ["Kabiraj"]
usernames = ["kabiraj"]
passwords = ["hashed_password_here"]  # Use hashed passwords
...
This ensures only authorized users can see or access the GitHub link.

Option 3: Don’t hardcode the link at all
You could store your GitHub repo link in an environment variable, and only read it when a secret key or password is entered. This way, even someone reading the code won’t see the link directly.

If you want, I can update your full working-hours app to include a password-protected GitHub link so only you can access it, while keeping the rest of the app fully functional.
...
ChatGPT said:
Got it! I’ll provide a full Streamlit app based on your latest working-hours tracker with:

Edit and save per day

Reset all data with confirmation

Responsive styling for mobile/tablet

GitHub link protected by a secret key
    ...
    for day in days:
        for suffix in ['start_hr', 'start_min', 'start_ampm', 'break', 'end_hr', 'end_min', 'end_ampm']:
            ...
            if key not in st.session_state:
                ...
                if not match.empty:
                    col_map = {
                        'start_hr': 'Start_hr', 'start_min': 'Start_min', 'start_ampm': 'Start_ampm',
                        'break': 'Break', 'end_hr': 'End_hr', 'end_min': 'End_min', 'end_ampm': 'End_ampm'
                    }
                ...
                if saved_val is not None and not pd.isna(saved_val):
                    ...
                    st.session_state[key] = 0.5 if suffix=='break' else 9 if 'hr' in suffix else 0 if 'min' in suffix else 'AM'
...
st.markdown("""
<style>
.title {text-align:center;color:#2E8B57;font-weight:700;font-size:2.5rem;margin-bottom:1rem;}
.employee-section {border-radius:12px;padding:1rem 1.5rem;margin-bottom:1.5rem;box-shadow:0 4px 8px rgba(46,139,87,0.15);}
.employee-0 { background-color:#e6f2e6; }
.employee-1 { background-color:#d9ead3; }
.employee-2 { background-color:#cde6d0; }
.header-row {display:flex;padding-bottom:8px;border-bottom:2px solid #2E8B57;font-weight:700;color:#2E8B57;font-size:1.1rem;margin-bottom:8px;align-items:center;gap:1rem;flex-wrap:nowrap;justify-content:space-between;}
.header-label-day {min-width:110px;text-align:left;padding-left:5px;white-space:nowrap;flex:1;}
.header-label-start {flex:2;text-align:left;white-space:nowrap;padding-left:10px;}
.header-label-break {flex:1;text-align:center;white-space:nowrap;}
.header-label-end {flex:2;text-align:right;white-space:nowrap;padding-right:10px;}
.header-label-right {min-width:180px;text-align:right;padding-right:5px;white-space:nowrap;flex:1;}
.day-row {display:flex;align-items:center;gap:1rem;padding:6px 0;border-bottom:1px solid #a3c9a5;font-size:0.95rem;flex-wrap:nowrap;justify-content:space-between;}
.day-label {min-width:110px;font-weight:600;color:#3a7d44;white-space:nowrap;text-align:left;padding-left:5px;flex:1;}
select,input[type=number]{border-radius:5px;border:1px solid #ccc;padding:5px 6px;font-size:0.9rem;width:48px;text-align:center;}
input[type=number]{width:70px;}
.hours-display {min-width:130px;text-align:right;font-weight:700;color:#2E8B57;font-size:1rem;padding-right:5px;flex:1;}
.edit-highlight {background-color:#fff3cd;border-radius:6px;padding:2px 5px;}
@media (max-width:768px){.header-row,.day-row{flex-wrap:wrap;font-size:0.85rem;}select,input[type=number]{width:60px !important;font-size:0.85rem;}.hours-display{text-align:left;min-width:auto;padding-left:5px;}}
@media (max-width:480px){.header-row div,.day-row div{flex:100%;text-align:left !important;}}
</style>
...
if st.button("🔄 Reset All Data"):
    confirm_reset = st.checkbox("⚠️ Confirm delete all data (cannot be undone)", key="confirm_reset")
    if confirm_reset:
        reset_all_data()
...
if secret_key == "MY_SECRET_KEY":  # Change to your secret
    ...
    if secret_key:
        st.warning("Incorrect secret key.")
